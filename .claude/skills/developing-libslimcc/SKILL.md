...
- **x86_64 glibc: 95/103**. The 8 fails are by-design rejections: `asm`, `inline_asm`, `tls`, `tls2`, `attr_weak`, `builtin_return_address`, `function2`, `xxxof_vmtype` (the last two: paramless C23 variadics — MIR requires ≥1 named arg).
- **aarch64, glibc AND musl, identical lists: 89/103** = the 8 above + 6 x86-test-assumption failures that gcc-on-aarch64 fails identically: `cast.c`, `literal.c`, `function.c`, `bitfield2.c` (plain char is unsigned per AAPCS64), `float2.c` (x87 layout memcmp), `unicode.c` (asserts `L'\xffffffff'>>31 == -1` but aarch64 wchar_t is unsigned).
