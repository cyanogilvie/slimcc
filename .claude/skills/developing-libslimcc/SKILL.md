...
- **aarch64, glibc AND musl, identical lists: 92/103** (confirmed 2026-06-12 on both EC2 boxes) = the 4 above + 6 x86-test-assumption failures that gcc-on-aarch64 fails identically: `cast.c`, `literal.c`, `function.c`, `bitfield2.c` (plain char is unsigned per AAPCS64), `float2.c` (x87 layout memcmp), `unicode.c` (asserts `L'\xffffffff'>>31 == -1` but aarch64 wchar_t is unsigned) — plus `function2.c`: an aarch64 MIR-layer vararg bug (>16-byte by-value struct vararg mixed with FP varargs desyncs the va_list cursors), unmasked by the paramless-variadic fixes (the file used to fail to compile, so this never ran; not a regression). Repro + analysis: `notes/mir-backend/aarch64-va-bigstruct-repro.c`; x86-64 immune; c2m can't oracle it ("Wrong alias number" on big-struct va_arg, separate c2m bug). Fork fix + upstream filing pending.
